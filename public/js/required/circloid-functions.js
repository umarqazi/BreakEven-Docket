	...
	$(".navbar-toggle").on("click", function(){
		if($("#left-column").height() > 0){
			if($("#left-column").hasClass("menu-open")){
				$("#left-column").animate({"left":"-110px"}, 200, "easeInQuart",function(){
					$(this).removeClass("menu-open");
					$(this).removeAttr("style");
				});
			}else{
				$("#left-column").animate({"left":"0"}, 500, "easeOutQuart",function(){
		...
		if($("#mainnav-horizontal-inner").height() > 0){
			if($("#mainnav-horizontal-inner").hasClass("menu-open")){
				$("#mainnav-horizontal-inner").slideUp(500, 'easeInOutQuart', function(){
					$(this).removeClass("menu-open");
					$(this).removeAttr("style");
				});
			}else{
				$("#mainnav-horizontal-inner").slideDown(500, 'easeInOutQuart', function(){
					$(this).addClass("menu-open");
				});
			}
		}
...
function circloidNotificationAlert(options){
	...
	if($(".header-notifications").hasClass("animate")){
		...
		var count = $('.notifications-alert').length;
		var duration = 200;
		var finalDelay = (count * duration) + 300;
	...
	$(document).on('click', function(event) {
		if (!$(event.target).closest('.notifications-alert').length) {
			$(".notifications-alert").children("ul").animate({"top":"53px", "opacity":"hide"}, 200, "easeInQuart",function(){
				$(this).removeAttr("style");
				$(this).parent("li").removeClass("menu-open");
			});
		}
...
function circloidRevealBugFix(){
	// Reveal Badges
	setTimeout(function(){
		$(".notifications-alert > .badge").animate({"opacity":"show"}, 500, "easeOutQuart");
